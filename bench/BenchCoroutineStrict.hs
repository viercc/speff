module BenchCoroutineStrict where
...
sumM :: (Monad m, Num b) => [a] -> (a -> m b) -> m b
sumM as f = go 0 as
  where
    go !acc [] = pure acc
    go !acc (a : as') = f a >>= \b -> go (acc + b) as'

programSp :: S.Yield Int Int S.:> es => Int -> S.Eff es Int
programSp upbound =
    sumM [1..upbound] \i -> S.yield i
{-# NOINLINE programSp #-}
...
programMp n = sumM [0..n] $ \i -> M.perform M.yield i
{-# NOINLINE programMp #-}
...
programFreer upbound =
    sumM [1..upbound] \i -> FS.yield i id
...
programEff upbound =
    sumM [1..upbound] \i -> E.yield @Int @Int i
{-# NOINLINE programEff #-}
